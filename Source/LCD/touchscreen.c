...
uint32_t LCD_TS_Read(PT_T * position) {
	...
	if (b>0) {
		...
		//x conv queue
		thread_queue[0].valid = 1;
		thread_queue[0].ChannelNum = LCD_TS_YU_CHANNEL;
		//y conv queue
		thread_queue[1].valid = 1;
		thread_queue[1].ChannelNum = LCD_TS_XL_CHANNEL;
		
		osEventFlagsWait(evflags_id, 1, osFlagsWaitAny, osWaitForever);
		
		x = thread_queue[0].result;
		y = thread_queue[1].result;
